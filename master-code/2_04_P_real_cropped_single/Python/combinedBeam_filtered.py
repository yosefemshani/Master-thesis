...
# Load the TOPAS trajectory data
trajectory_data = np.loadtxt('trajectory_TOPAS_water_filtered.csv', delimiter=',')
...
plt.plot(trajectory_data[:, 0], trajectory_data[:, 1], color='darkgreen', linewidth=2, alpha=0.85, label='TOPAS Trajectory')

# Load the MATLAB trajectory data
trajectory_MATLAB_data = np.loadtxt('trajectory_mm.csv', delimiter=',')

# Plot the trajectory
plt.plot(trajectory_MATLAB_data[:, 0], trajectory_MATLAB_data[:, 1], 'r-', linewidth=3.8, alpha=0.5, label='MATLAB Trajectory')  # Plot the trajectory
plt.legend()
plt.show()
plt.close()
