classdef ProtonSimulation_inverse
    properties
        % Constants
        mp_MeV = 938.272013;
        ...
        end_position; % Desired end position [mm]
    ...
    methods
        function obj = ProtonSimulation_inverse(SPR_values, end_position, B, grid_step, total_distance, initial_position, initial_velocity)
            ...
            obj.E_MeV = 0.49; % Start with a lower bound for energy
        end
        
        function obj = initializeStep(obj)
            % Initialize the first step with given energy
            gamma0 = 1 + obj.E_MeV / obj.atomic_mass_unit_MeV_c2;
            ...
            obj.velocities(:, 1) = v;
            obj.energies(1) = obj.E_MeV; % Store initial energy
        end
    ...
    %disp(['dE/dx is ', num2str(energy_loss_keV_um), ' [keV/μm]']);
        ...
        function energy = findRequiredEnergy(obj)
            % Iteratively adjust energy to match the desired end position
            tolerance = 0.01; % Tolerance for end-position accuracy
            max_iterations = 1000;
            iteration = 0;
            step = obj.num_steps; % Use last step to check end position
            while iteration < max_iterations
                obj.initializeStep();
                obj.simulate();
                
                % Calculate the distance to the desired end-position
                distance_to_end = norm(obj.positions(:, step) - obj.end_position);
                if distance_to_end < tolerance
                    break;
                else
                    % Increase energy and try again
                    obj.E_MeV = obj.E_MeV + 0.1; % Increment energy
                    iteration = iteration + 1;
                end
            end
            
            if iteration >= max_iterations
                error('Failed to find the required energy within the maximum number of iterations.');
            end
