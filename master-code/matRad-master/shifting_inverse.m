...
stopping_energy_MeV = 100;
E0_MeV = 0.49;
B = [0; 0; 1.5]; % [T]
desired_final_position = [10; 15; 0];
...
v = [0.000000000001; 0; 0]; % Velocity vector with velocity in x direction!
...
% Initial conditions
positions(:,end) = desired_final_position; % Initial position (assuming starting from origin)
...
for step = num_steps-1:-1:1
    % Check if the energy level has reached or exceeded the stopping energy level
    if E_MeV >= stopping_energy_MeV
        disp(['Energy level reached the stopping energy of ', num2str(stopping_energy_MeV), ' MeV at step ', num2str(step)]);
        break;
    end
    ...
    % Update velocity
    velocities(:, step) = velocities(:, step + 1) + a * time_step;
...
function energy_loss_MeV_cm = calculate_energy_loss(E_MeV)
    ...
    disp(['dE/dx is ', num2str(energy_loss_MeV_m), ' [MeV/m]']);
