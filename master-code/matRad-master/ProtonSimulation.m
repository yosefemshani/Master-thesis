classdef ProtonSimulation
    ...
    methods
        function obj = ProtonSimulation(SPR_values, E0_MeV, B, grid_step, total_distance, initial_position, initial_velocity)
        ...
        function obj = simulate(obj)
            ...
            for step = 2:obj.num_steps
                time_step = obj.grid_step / norm(obj.velocities(:, step - 1));
                F = obj.q * cross(obj.velocities(:, step - 1), obj.B);
                a = F / obj.mp;
                ...
                if obj.E_MeV < 0.49
                    ...
                    x_pos = obj.positions(1, step-1) * 10;
                    y_pos = obj.positions(2, step-1) * 10;
                ...
                vx_new = obj.velocities(1, step-1) + a(1) * time_step;
                vy_new = obj.velocities(2, step-1) + a(2) * time_step;
                vz_new = obj.velocities(3, step-1) + a(3) * time_step;
                obj.velocities(:, step) = [vx_new; vy_new; vz_new];
                
                x_distance = obj.x_distance + abs(vx_new) * time_step;
                if x_distance >= obj.grid_step
                    full_steps = floor(x_distance / obj.grid_step);
                    x_new = obj.positions(1, step - 1) + sign(vx_new) * full_steps * obj.grid_step;
                    x_distance = x_distance - full_steps * obj.grid_step;
                    y_new = obj.positions(2, step - 1) + vy_new * (full_steps * obj.grid_step / abs(vx_new));
                    z_new = obj.positions(3, step - 1) + vz_new * (full_steps * obj.grid_step / abs(vx_new));
                    obj.positions(:, step) = [x_new; y_new; z_new];
                else
                    x_new = obj.positions(1, step - 1) + sign(vx_new) * obj.grid_step;
                    y_new = obj.positions(2, step - 1) + vy_new * (obj.grid_step / abs(vx_new));
                    z_new = obj.positions(3, step - 1) + vz_new * (obj.grid_step / abs(vx_new));
                    obj.positions(:, step) = [x_new; y_new; z_new];
                    obj.x_distance = 0;
                end
                
                obj.times(step) = obj.times(step - 1) + time_step;
                obj.accelerations(:, step) = a;
            end
        end
