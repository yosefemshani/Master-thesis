%% Integrating magnetic field grid size model with energy loss 
%% Author: Yosef Emshani, 27.02.2024 12:37
...
% updated once that proton travels grid step!
...
for step = 2:num_steps
    ...
    % Print out Lorentz force components for DEBUGGING
    %disp(['Step: ', num2str(step)]);
    %disp(['Lorentz Force Fx: ', num2str(Fx)]);
    %disp(['Lorentz Force Fy: ', num2str(Fy)]);
    
    % Calculate energy loss
    energy_loss_MeV_cm = calculate_energy_loss(E_MeV);
    
    % Update energy
    E_MeV = E_MeV - energy_loss_MeV_cm * grid_step;
