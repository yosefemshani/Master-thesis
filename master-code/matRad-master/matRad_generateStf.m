...
for i = 1:size(voiTarget, 3)
    filename = sprintf('voiTarget_page_%d.csv', i);
    csvwrite(filename, voiTarget(:, :, i));
    disp(['voiTarget page ' num2str(i) ' has been saved as ' filename '!']);
end
...
for i = 1:length(pln.propStf.gantryAngles)
    ...
    csvwrite('rayPos.csv', rayPos');
    disp(['Ray positions have been saved as rayPos.csv!']);
