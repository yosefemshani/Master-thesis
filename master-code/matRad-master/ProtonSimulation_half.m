classdef ProtonSimulation_half
    ...
    methods
        ...
        function obj = initializeStep(obj)
            ...
            obj.positions(2, 1) = obj.positions(2, 1) * 1.09375;
            obj.velocities(:, 1) = v;
    ...
    I_eV = 78; % 75 eV for liquid water / 78 in TOPAS. for others, see https://github.com/libamtrack/library/blob/44dc48cfa977c05008ad12646d798d6c4b6ea504/include/AT_DataMaterial.h l. 197-203
    ...
    %disp(['dE/dx is ', num2str(energy_loss_keV_um), ' [keV/μm]']);
        ...
        function saveResults(obj)
            ...
            % Also, save the total trajectory length to a file or display it
            disp(['Total trajectory length s: ', num2str(obj.total_trajectory_length), ' mm']);
             ...
             % axis equal;
            %figure;
            %plot(filtered_positions(1, :)*10, filtered_energyloss()/max(filtered_energyloss()), 'b.-', 'LineWidth', 1);
            ...
            %yosef_x = filtered_positions(1, :)*10;
            %yosef_y = filtered_energyloss()/max(filtered_energyloss());
        end
        ...
        function num_non_zero_steps = displayStepRange(obj)
            % Method to display and return the number of steps with non-zero values
            non_zero_mask = any(obj.positions ~= 0, 1);
